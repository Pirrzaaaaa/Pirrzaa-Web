...
export async function requestPairingCode(sock) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      // Notice tambahan kalau pakai custom code
      if (customCode && customCode === finalCode) {
        console.log(
          "ℹ️  Catatan: WA kadang reject custom pairing code 'kreatif'.",
        );
        console.log(
          "    Kalau code di atas selalu salah meski ngetik benar:",
        );
        console.log(
          "    - Hapus folder ./session, ganti config.pairingCode ke null,",
        );
        console.log("      lalu start ulang. Library akan pakai code default.");
        console.log(
          "    - Atau set usePairingCode=false untuk pakai QR-CODE.",
        );
        console.log("");
      }
